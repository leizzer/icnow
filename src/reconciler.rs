...
pub fn reconcile_imports(db_path: &str) -> Result<()> {
    ...
    let query =
        "MATCH (f:File)-[r]->(i:Import) WHERE type(r) = 'REL_CONTAINS' RETURN f.id, i.id, i.name";
    ...
    for (src, target) in edges_to_create {
        let edge_id = format!("{src}::IMPORTS::{target}");
    ...
    for i_id in to_delete {
        ...
        let _ = conn.cypher(&format!(
            "MATCH (n) WHERE n.id = '{escaped}' DETACH DELETE n"
        ));
    }
