...
pub fn handle_trace_call_path(db_path: &str, req: TraceCallPathRequest) -> Result<String, String> {
    ...
    let q = if !req.end_node_id.is_empty() {
        format!("MATCH (s:Symbol {{id: '{}'}})-[e:CALLS*1..{}]->(t:Symbol {{id: '{}'}}) RETURN e", req.start_node_id.replace("'", "''"), max_depth, req.end_node_id.replace("'", "''"))
    } else {
        format!("MATCH (s:Symbol {{id: '{}'}})-[e:CALLS*1..{}]->(t:Symbol) RETURN e, t.id AS target_id", req.start_node_id.replace("'", "''"), max_depth)
    };

    let mut res = conn.query(&q).map_err(|e| format!("Tracing query failed: {e}"))?;
...
pub fn handle_traverse_graph(db_path: &str, req: crate::tools::TraverseGraphRequest) -> Result<String, String> {
    let conn = crate::open_db_connection(db_path).map_err(|e| format!("Failed to open DB: {e}"))?;
    let max_depth = req.max_depth.unwrap_or(2);
    let q = format!("MATCH (s:Symbol {{id: '{}'}})-[r*1..{}]-(t) RETURN r, t.id AS target_id LIMIT 100", req.node_id.replace("'", "''"), max_depth);
    let mut res = conn.query(&q).map_err(|e| format!("Failed to traverse graph: {e}"))?;
    crate::tools::format_cypher_result(&mut res)
}
